...
class Stringalong {
  ...
  constructor(source, opts = {}) {
    ...
    this.meta = {
      ...
      forceUnique: true, allRoots: false, weightedChances: false, includes: []
    };
  ...
  parse(source) {
    ...
    for (const raw of lines) {
      ...
      if (line[0] === '$') {
        ...
        if (dl === 'weighted chances'){ if (!inIncludes) {this.meta.weightedChances = true;} continue; }
        if (dl.startsWith('include ')){ this.meta.includes.push(dir.slice(8).trim()); continue; }

        // List declaration
        let name = dir, isRoot = false, append = false;
        if (name.includes('>')) { isRoot = true; name = name.replace(/>/g, '').trimEnd(); }
        if (name[0] === '+') { append = true; name = name.slice(1); }

        const attrs = {};
        if (name.endsWith('}') && name.includes(' {')) {
          const bi = name.indexOf(' {');
          for (const part of name.slice(bi + 2, -1).replace(/}\s*{/g, '}{').split('}{')) {
            if (part.includes(':')) {
              const ci = part.indexOf(':');
              attrs[part.slice(0, ci).toLowerCase().trim()] = part.slice(ci + 1).trim();
            } else {
        ...
        if (append && this.lists.has(name)) {
          ...
          current.items = [];
          current.attrs = {};
          current.megahal = false;
          current.megahalOrder = 5;
          delete current.brain;
        } else {
          current = { name, items: [], root: false, attrs: {} };
          this.lists.set(name, current);
          this.listOrder.push(name);
        }

        current.attrs = { ...current.attrs, ...attrs };
        if (current.attrs.megahal) {
          current.megahal = true;
          const parsedOrder = parseInt(current.attrs.megahal);
          current.megahalOrder = isNaN(parsedOrder) ? 5 : parsedOrder;
        }
      ...
      // List item
      if (current) {
        current.items.push(this._parseItem(line));
      }
    }
    return this;
  }
  ...
  /**
   * @param {{ count?: number, seed?: string|null, root?: string }} [options]
   */
  generate({ count, seed, root } = {}) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const text = item ? (item.literal ? item.text : this._eval(item.text, ctx)) : '';
  ...
  // Pass/fail filter (default): each item independently rolls against its
  // chance to enter the candidate pool, then one is picked uniformly.
  // {50%} = 50% chance of being in the pool. Untagged items always pass.
  // Falls back to full list if nothing passes.
  _pickFiltered(items, rng) {
    const pool = items.filter(it => rng() < (it.tags?.chance ?? 1));
    return this._pick(pool.length ? pool : items, rng);
  }

  // Weighted pick (opt-in via $weighted chances): {90%} means this item
  // occupies 90% of the deck. Untagged items split whatever remains equally.
  // If explicit percentages exceed 100%, they are used as relative weights.
  _pickWeighted(items, rng) {
    let explicitTotal = 0, untaggedCount = 0;
    for (const it of items) {
      const c = it.tags?.chance ?? 1;
      if (c < 1) {explicitTotal += c;}
      else {untaggedCount++;}
    }
    const remainder = Math.max(0, 1 - explicitTotal);
    const untaggedWeight = untaggedCount > 0 ? remainder / untaggedCount : 0;

    const weights = items.map(it => {
      const c = it.tags?.chance ?? 1;
      return c < 1 ? c : untaggedWeight;
    });
  ...
  _pickFromList(list, ctx, unique) {
    if (list.megahal) {
      if (list.brain === undefined) {
        const MegaHal = Stringalong.MegaHal || globalThis.MegaHal;
        if (!MegaHal) {
          this._warn('MegaHal is not loaded. Falling back to default list picking.');
          list.brain = null;
        } else {
          const brain = new MegaHal(list.megahalOrder ?? 5);
          brain.setLimit({ timeout: 0, maxIterations: 1 });
          for (const item of list.items) {
            brain.learn(item.text);
          }
          list.brain = brain;
        }
      }
      ...
      if (list.brain) {
        list.brain.rng = {
          randomRange: (min, max) => min + Math.floor(ctx.rng() * (max - min))
        };
        const utterance = list.brain.generate('');
        list.brain.rng = null;
        if (utterance !== null && (!unique || !ctx.uniques.has(utterance))) {
          return { text: utterance, tags: { chance: 1, attrs: {} }, literal: true };
        }
      }
    }
    ...
    if (unique) {
      const pool = items.filter(it => !ctx.uniques.has(it.text));
      if (pool.length) {items = pool;}
    }
    return this.meta.weightedChances
      ? this._pickWeighted(items, ctx.rng)
      : this._pickFiltered(items, ctx.rng);
  }
  ...
  _evalTagInner(rawTag, ctx, toObj = false) {
    ...
    const resolvedAs = this._resolveAs(as, ctx);
    ...
    if (tag[0] === '#') {
      ...
      if (!stored) {
        this._warn('Unknown identifier: ' + tag);
      } else {
        for (let i = 0; i < times; i++) {
          if (typeof stored === 'string') {
            ...
            if (resolvedAs && obj.tags?.attrs[resolvedAs] != null) {
              ...
              out += this._eval(fallback, ctx);
            } else {
              out += this._eval(obj.text, ctx);
            }
          }
        }
      }
      ...
      for (let i = 0; i < times; i++) {
        ...
        else {text = item.literal ? item.text : this._eval(item.text, ctx);}
    ...
    // Strip leading determiner (article, possessive, demonstrative)
    if (bare) {out = out.replace(/^(a|an|the|my|your|his|her|its|our|their|this|that|these|those)\s+/i, '');}
...
if (typeof module !== 'undefined' && module.exports) {
  module.exports = Stringalong;
} else {
  const amdDefine = /** @type {any} */(root).define;
  if (typeof amdDefine === 'function' && amdDefine.amd) {
    amdDefine(() => Stringalong);
  } else {
    /** @type {any} */(root).Stringalong = Stringalong;
  }
}
