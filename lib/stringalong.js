...
class Stringalong {
  ...
  _pickFromList(list, ctx, unique) {
    const pool = list.items.filter(it => ctx.rng() < it.tags.chance);
    if (!pool.length) return null; // all items failed chance — intentional empty output
    return unique ? this._pickUnique(pool, ctx.uniques, ctx.rng) : this._pick(pool, ctx.rng);
  }
